...
class TaskService extends BaseService
{
    ...
    {
        $tasks = $this->getTaskRepository()->searchTasks($tasksStatus);
    ...
    public function updateTask($input, $taskId)
    {
        ...
        if (!isset($input['name']) && !isset($input['description']) && !isset($input['status'])) {
            throw new TaskException(TaskException::TASK_INFO_REQUIRED, 400);
        }
        $data = [];
        $data['name'] = vs::validateNameOnUpdateTask($input, $checkTask);
        $data['description'] = vs::validateDescriptionOnUpdateTask($input, $checkTask);
