...
abstract class TaskValidation extends BaseValidation
{
    ...
    public static function validateInputOnCreateTask($input)
    {
        ...
        return ['name' => $task, 'description' => $description, 'status' => $status];
    }
     ...
     * @return string
     */
    public static function validateDescriptionOnUpdateTask($input, $task)
    {
        $description = $task->description;
        if (isset($input['description'])) {
            $description = self::validateTaskDescription($input['description']);
        }
